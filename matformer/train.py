...
class CustomMetric_lattice(Metric):
    ...
    def update(self, output):
        ...
        self._mae.append(MAE_loss(y_pred["lattice"], y_gt[0]["lattice"].t().view(-1,3,3)).item())
...
class CustomMetric_position(Metric):
    ...
    def update(self, output):
        ...
        self._mae.append(MAE_loss(y_pred["positions"], y_gt[0]["positions"].t()).item())
...
class CustomMetric_atom(Metric):
    ...
    def update(self, output):
        ...
        label = y_gt_dict[0]["atoms"]
        mask = y_gt_dict[0]["mask"]
...
def train_dgl(
    ...
    if pre_train:
        class Criterion(nn.Module):
            ...
            def forward(self, y_pred, y_gt):
                ...
                if "atoms" in y_pred.keys():
                    atom_loss = self.loss_atoms(y_pred["atoms"], y_gt[0]["atoms"], y_gt[0]["mask"])
                    all_loss.append(atom_loss)
                if "positions" in y_pred.keys():
                    position_loss = self.l1(y_pred["positions"], y_gt[0]["positions"].t())
                    all_loss.append(position_loss)
                if "lattice" in y_pred.keys():
                    lattice_loss = self.l1(y_pred["lattice"], y_gt[0]["lattice"].t().view(-1,3,3))
                    all_loss.append(lattice_loss)
                loss = all_loss[0]
                if len(all_loss)>1:
                    for i in range(1, len(all_loss)):
                        loss+=all_loss[i]
        ...
        class Acc(nn.Module):
            ...
            def forward(self, y_pred, mask, y):
                #y_pred, (mask, y) = output
                _, y_pred_class = torch.max(y_pred, 1)
                # y_pred = torch.round(torch.exp(y_pred))
                # print ('output',y_pred)
                predd = torch.masked_select(y_pred_class, mask==1)
                targett =  torch.masked_select(y, mask==1)
                accuracy = (predd == targett).sum().item() / len(targett)
                return accuracy
        metrics = {
            "loss": Loss(criterion),
        }
    ...
    def log_results(engine):
        ...
        if config.progress:
            pbar = ProgressBar()
            if pre_train:
                pbar.log_message(f"Val_loss: {vmetrics['loss']:.4f}")
                pbar.log_message(f"Train_loss: {vmetrics['loss']:.4f}")
    ...
    if pre_train:
        ...
        with torch.no_grad():
            for dat in test_loader:
                ...
                targets.append(target)
                predictions.append(out_data)
                for t, p in zip(target, out_data):
                    f.write("%6f, %6f\n" % (t, p))
            mae = mean_absolute_error(np.array(targets), np.array(predictions)) * std_train
            f.write("test_mae, %d, %d\n" % ((mae), (mae)))
        f.close()
